...
export type CardPrompt = { id:string; title:string; body:string; summary:string; category:string; tags:string[]; variables:Variable[]; model_hint:string[]; use_count:number; last_used:string|null; source:string; fork_of:string|null; updated_at:string; body_en?:string|null; created_at?:string; group_id?:string; version_no?:number; version_note?:string; versions?:CardPrompt[]; highlight?:{text:string;ranges:[number,number][]} };
export type Library = { items:CardPrompt[]; total:number; uses:number; categories:{name:string;count:number;fixed?:boolean}[]; tags:string[]; mode:'local'|'cloud'; degraded:boolean; warning?:string; manage?:boolean; stamp?:string; synced_at?:string };
