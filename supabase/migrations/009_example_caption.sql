...
-- 009：範例說明可以事後修改
--
-- 原本只有「上傳時一起帶上 caption」這一條路。上傳完才想補說明的人，會在畫面上
-- 找不到任何入口——線上資料裡好幾個範例的 caption 就是空字串。
-- 這裡加一個 example_caption，並沿用 008 的維護者閘門（改的是既有內容）。
--
...
BEGIN
 IF op IN ('delete','delete_many','merge','categories_save','example_remove','example_caption',
